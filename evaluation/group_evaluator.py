...
class GroupEvaluator(BaseEvaluator):
    ...
    def evaluate(self, model, dataset, nsteps=None):
        ...
        for i, evaluator in enumerate(self.evaluators):
            ...
            with torch.no_grad():
                _ = evaluator.evaluate(model, dataset, nsteps)
                psnr, ssim, LPIPS_vgg, LPIPS_alex, score_rand, score_rec = evaluator.evaluate_metrics(model, dataset, nsteps)


                metrics = {'psnr':psnr, 'ssim':ssim, 'fid':score_rec, 'fid_swap':score_rand, 'LPIPS_vgg':LPIPS_vgg, 'LPIPS_alex':LPIPS_alex}

                print('Evaluation metrics: ', psnr, ssim, score_rec, score_rand)
