...
from models.networks.stylegan2_vec_layers import ConvLayer, ToRGB, EqualLinear, StyledConv
...
class GeneratorModulation(torch.nn.Module):
    ...
    def forward(self, x, style):
        ...
        if style.ndimension() <= 2:
            ...
            style = F.interpolate(style, size=(x.size(2), x.size(3)), mode='bilinear', align_corners=True)

            return x * (1 * self.scale(style)) + self.bias(style)


class StyleGAN2ResnetGeneratorVaVector(BaseNetwork):
    ...
    def forward(self, spatial_code, h, v):

        # if spatial_code != None:
        spatial_code = util.normalize(spatial_code)

        if len(h.shape)<2:
            va_code = torch.cat((h.unsqueeze(0), v.unsqueeze(0)), dim=1)
        else:
            va_code = torch.cat((h,v), dim=1)

        # Use viewwing angle maps
        x = self.SpatialCodeModulation(spatial_code, va_code)


        for i in range(self.opt.netG_num_base_resnet_layers):
            resblock = getattr(self, "HeadResnetBlock%d" % i)
            x = resblock(x, va_code)

        for j in range(self.opt.netE_num_downsampling_sp):
            key_name = 2 ** (4 + j)
            upsampling_layer = getattr(self, "UpsamplingResBlock%d" % key_name)
            x = upsampling_layer(x, va_code)

        rgb = self.ToRGB(x, va_code, None)
