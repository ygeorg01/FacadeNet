...
class EqualLinear(nn.Module):
    def __init__(
        ...
        # 2-D style
        # self.weight = nn.Parameter(torch.randn(out_dim, in_dim, 3, 3).div_(lr_mul))
    ...
    def forward(self, input):
        ...
        # print('input shape ', input.shape, self.weight.shape)
        if self.activation:
            # if input.dim() > 2:
            ...
            # if input.dim() > 2:
                ...
                # )
                # print('conv shape ', input.shape, self.weight.shape)
                # out = F.conv2d(input, self.weight * self.scale,
                #                bias=self.bias * self.lr_mul, padding=1,
                #                )
            # else:
                out = F.linear(
...
class ModulatedGenConv2d(nn.Module):
    def __init__(
        self,
        in_channel,
        out_channel,
        ...
        if kernel_size == 3:
            self.modulation = EqualConv2d(style_dim, in_channel, kernel_size, padding=1)
        else:
            self.modulation = EqualConv2d(style_dim, in_channel, kernel_size, padding=0)

        self.demodulate = demodulate
        self.new_demodulation = True

        self.weight_gamma = nn.Parameter(
            torch.randn(in_channel, in_channel, 3, 3)
        )
    ...
    def forward(self, input, style):
        ...
        if style.dim() > 2:
            style = F.interpolate(style, size=(input.size(2), input.size(3)), mode='bilinear', align_corners=True)

            style = self.modulation(style)
            gamma = F.conv2d(style, self.weight_gamma, padding=1)
            if self.demodulate:
                # style = style * torch.rsqrt(style.pow(2).mean([2], keepdim=True) + 1e-8)
                gamma = gamma * torch.rsqrt(gamma.pow(2).mean([2], keepdim=True) + 1e-8)
                # beta = -beta * torch.rsqrt(beta.pow(2).mean([2], keepdim=True) + 1e-8)

            # Convolve style matrix
...
class GenToRGB(nn.Module):
    def __init__(self, in_channel, style_dim, upsample=True, blur_kernel=[1, 3, 3, 1]):
        ...
        self.conv = ModulatedGenConv2d(in_channel, 3, 1, style_dim, demodulate=False)
        self.bias = nn.Parameter(torch.zeros(1, 3, 1, 1))
...
class DiscriminatorVA(nn.Module):
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1]):
        ...
        convs = [('0', ConvLayer(5, channels[size], 1))]
        # convs = [('0', ConvLayer(4, channels[size], 1))]
        ...
        self.vp_predictions = nn.Sequential(
            EqualLinear(channels[4] * (side_length ** 2), channels[4]),
            EqualLinear(channels[4], 2),
        )
