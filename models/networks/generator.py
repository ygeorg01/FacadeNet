...
from models.networks.stylegan2_layers import ConvLayer, ToRGB, EqualLinear, StyledConv, ToVH
...
class StyleGAN2ResnetGenerator(BaseNetwork):
    ...
    def __init__(self, opt):
        ...
        vh_layer = ToVH(out_channel, self.global_code_ch,
                           blur_kernel=blur_kernel)
        self.add_module("ToRGB", last_layer)
        self.add_module("ToVH", vh_layer)
    ...
    def forward(self, spatial_code, global_code):
        ...
        vh = self.ToVH(x, global_code, None)

        out = torch.cat((rgb, vh), dim=1)
