...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    Parameters:
        webpage (the HTML class) -- the HTML webpage class that stores these imaegs (see html_c.py for more details)
...
class Visualizer():
    ...
    def display_current_results(self, visuals, epoch,
        ...
        if 'mask' in visuals.keys():
            # print('Visuals hereee!!!: ',visuals['mask'].shape)
            print('masks dir: ', os.path.join(self.checkpoints_dir, 'epoch%.3d_masks.png' % (epoch)))
            image_numpy = util.tensor2im(visuals['mask'])
            img_path = os.path.join(self.checkpoints_dir, 'epoch%.3d_masks.png' % (epoch))
            util.save_image(image_numpy, img_path)

            # print('Visuals hereee!!!: ',visuals.keys())
            image_numpy = util.tensor2im(visuals['real'])
            img_path = os.path.join(self.checkpoints_dir, 'epoch%.3d_reals.png' % (epoch))
            util.save_image(image_numpy, img_path)
        ...
        if self.display_id > 0:  # show images in the browser using visdom
            ...
            if ncols > 0:        # show all the images in one visdom panel
                ...
                for label, image in visuals.items():
                    ...
                    if image is None:
                        print('Image for labels %s is %s' % (label, image))
                        continue
                    if image.size(3) < 64:
