...
def get_transform(opt, params=None, grayscale=False, make_power=False, method=Image.BICUBIC, convert=True):
    transform_list = []
    ...
    if 'resize' in opt.preprocess:
        ...
        transform_list.append(
            transforms.Lambda(lambda img: __scale_longside(img, opt.load_size, opt.crop_size, method)))
    ...
    if 'centercrop' in opt.preprocess:
        ...
        if params is None or 'crop_pos' not in params:
            ...
            if convert:
                ...
                transform_list.append(transforms.Lambda(lambda img: __crop_tensor(img, params['crop_pos'], opt.crop_size)))
...
def __crop_tensor(img, pos, size):
    ow, oh = img.shape[1:]
    x1, y1 = pos
    tw = th = size
    if (ow > tw or oh > th):
